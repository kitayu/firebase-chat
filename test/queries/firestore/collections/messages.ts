...
vi.mock('firebase/firestore', async () => {
	const firestore = await vi.importActual<object>(
		'firebase/firestore'
	);
	...
	describe('messagesQuery', async () => {
		...
		beforeEach(async () => {
			...
			await env.withSecurityRulesDisabled(async (context) => {
				...
				const { Timestamp, collection } = await import('firebase/firestore');
				const { messageFactory } = await import(
					'@/../test/factories/message'
				);
		...
		it('createdAtの昇順にソートされたメッセージが取得できる', async () => {
			...
			expect(
				snapshot.docs.map((doc) => doc.data().content)
			).toEqual([
					'1番目のメッセージ',
					'2番目のメッセージ',
					'3番目のメッセージ',
